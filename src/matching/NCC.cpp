...
vector<DMatch> NCC::NCCMatching(vector<float> &mean1, vector<float> &mean2, vector<vector<float>> &descriptor1, vector<vector<float>> &descriptor2, float threshold)
{
    ...
    for (int i = 0; i < minimum; i++)
    {
        ...
        {
            float numenator = 0.0f;
            float dominator = 0.0f;
            float div = 0.0f;
            for (int j = 0; j < 16; j++)
            {
                numenator += ((float)des1[i][j] - m1[i]) * ((float)des2[k][j] - m2[k]);
                dominator += (float)sqrt(pow(((float)des1[i][j] - m1[i]), 2) * pow(((float)des2[k][j] - m2[k]), 2));
            }
            div = float((numenator / dominator));
