...
#include "../../include/edges/prewitt.h"

using namespace std;
using namespace cv;

prewitt::prewitt(Mat image)
{
    img = image;
    gaussian gauss(img);
    greyscale grey(img);
    ...
    pFiltered = Mat(prewittFilter()); //Prewitt Filter
}

Mat prewitt::prewittFilter()
{
    ...
    for (int i = size; i < gFiltered.rows - size; i++)
    {
        for (int j = size; j < gFiltered.cols - size; j++)
        {
            ...
            for (int x = 0; x < xFilter.size(); x++)
                ...
                {
                    sumx += xFilter[x][y] * (double)(gFiltered.at<uchar>(i + x - size, j + y - size)); //Prewitt_X Filter Value
                    sumy += yFilter[x][y] * (double)(gFiltered.at<uchar>(i + x - size, j + y - size)); //Prewitt_Y Filter Value
                }
