...
Polygon::Polygon() {
	...
	void Polygon::update(const Mat &G) {
		regularPoints = rechantillonage();
	...
	void Polygon::replacePoint(int i, Point2d p) {
		regularPoints.at(i) = p;
	}

	Mat Polygon::drawPolygon(const Mat &Image) {
		...
		imshow("Please Define Polygon", Image);
		return Image;
	}

	void Polygon::drawNormales(const Mat &Image) {
	...
	void Polygon::drawCurvatures(const Mat &Image) {
		if (points.size() < 3) return;
	...
	Point convexPoint(Point beginPoint, Point endPoint, double t) {
		return Point(t * endPoint.x + (1 - t) * beginPoint.x, t * endPoint.y + (1 - t) * beginPoint.y);
	}

	vector<Point2d> Polygon::rechantillonage() {
		int l = points.size();
	...
	vector<Vec2d> Polygon::computeTangentes() {
		int L = regularPoints.size();
		std::vector<Vec2d> tangentes(L);
		if (regularPoints.size() < 3) return tangentes;
	...
	vector<Vec2d> Polygon::computeNormales() {
		int L = regularPoints.size();
	...
	vector<Vec2d> Polygon::computeDeriv() {
		int L = regularPoints.size();
	...
	vector<double> Polygon::computeCurvature() {
		int L = regularPoints.size();
	...
	double Polygon::getCurvature(int i) {
		return curvature.at(i);
	}

	Vec2d Polygon::getNormale(int i) {
		return normales.at(i);
	}

	Point2d Polygon::getPoint(int i) {
		return regularPoints.at(i);
	}

	vector<Point2d> Polygon::deleteLoop() {
		int L = regularPoints.size();
		if (L < 2) return regularPoints;
	...
	bool segmentCut(Point2d p1, Point2d p2, Point2d q1, Point2d q2) {
		...
		if (xintersect == q1.x || xintersect == q2.x) return false;
		if (xintersect == p1.x || xintersect == p2.x) return false;
